...
class Facade {
  ...
  setupState = ([key, { fields }]) => {
    this[key] = this[key] || {}
    fields.forEach(this.setupStateField(key))
  }

  setupStateField = key => ({ name, type, allowed }) => {
    this[key][name] = {
      value: () => this.stores[key].getField(name),
      set: value => this.stores[key].setField({ name, value }),
    }
    if (type === 'enum') {
      this.setupEnumField({ key, name, allowed })
    }
  }

  setupEnumField({ key, name, allowed }) {
    this[key][name] = this[key][name] || {}
    this[key][name].is = this[key][name].is || {}
    allowed.forEach(allowedStr => {
      this[key][name].is[allowedStr] = () =>
        this.stores[key].fieldIs({ name, value: allowedStr })
    })
  }
